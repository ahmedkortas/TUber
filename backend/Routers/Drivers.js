...
router.post('/register', async (req, res) => {
    ...
    const emailExist = await Drivers.findOne({ where: {email: req.body.email}})
...
router.delete('/:id',async (req, res) => {
    await Drivers.findByPk(req.params.id).then((drivers) => {
        Drivers.destroy();
    }).then(() => {
        res.json("deleted");
    });
...
router.delete('/' ,async (req, res) => {
    await Drivers.destroy({where:{},truncate : true}).then(() => res.json("cleared"))
});
